...
SELECT f.nomf FROM f JOIN puf ON puf.nf = f.nf AND puf.nu = 2 AND puf.np = 7;

-- Donner le nom et la couleur des produits livrés par le fournisseur no 12.
SELECT p.nomp, p.couleur FROM p JOIN puf ON p.np = puf.np AND puf.nf = 12;
...
JOIN puf ON puf.nf = f.nf AND puf.nu = 5
JOIN p ON p.np = puf.np
